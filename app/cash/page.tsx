...
export default async function CashPage() {
  ...
  const [session, history, names] = await Promise.all([getCashSession(ctx), listCashSessions(ctx), profileNames(ctx)]);
  ...
  return (
    <CashRegister
      tenant={{ id: ctx.tenant.id, name: ctx.tenant.name, currency: ctx.tenant.currency, locale: ctx.tenant.locale, timezone: ctx.tenant.timezone }}
      role={ctx.role}
      session={session}
